...
def signin(request):
    ...
    response.set_cookie(jwt_cookie_name, tokens['access'], samesite=settings.HTTP_COOKIE_SAMESITE, secure=settings.USING_HTTPS, httponly=True, max_age=JWT_SETTINGS['ACCESS_TOKEN_LIFETIME'].total_seconds())
    return response
